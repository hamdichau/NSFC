...
def getResponse(resp=None,i=1):
    ...
    headers = {
        ...
        'method': 'POST',
        'cookie': '_gscu_1512923953=00785253fwctg711; JSESSIONID=EECB154A243E2EBD9F34AF8C6BA55075',
        'origin': 'http://npd.nsfc.gov.cn',
        'referer': 'http://npd.nsfc.gov.cn/projectSearch!search.action?project.applyCode=C',
...
html = json.loads(getResponse(i=1326).content)
for y in range(0, 13):
    applyCode = a.append(html["projectList"][y]["applyCode"])
    category = b.append(html["projectList"][y]["category"])
    categoryCode = c.append(html["projectList"][y]["categoryCode"])
...
df.to_csv('./NSFCbio.csv', index=False)
timenow = time.strftime(ISOTIMEFORMAT, time.localtime(time.time()))
